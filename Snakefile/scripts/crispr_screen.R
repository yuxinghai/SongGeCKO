setwd("/data5/zhoulab/yuxinghai/SongGeCKO/file/06_mageck/")
dir="/data5/zhoulab/yuxinghai/SongGeCKO/file/08_post_mageck_analysis/"
A1 <- read.table("screen_A1.csv",header = F,sep = "\t",quote = "\"")
A2 <- read.table("screen_A2.csv",header = F,sep = "\t",quote = "\"")
B1 <- read.table("screen_B1.csv",header = F,sep = "\t",quote = "\"")
B2 <- read.table("screen_B2.csv",header = F,sep = "\t",quote = "\"")
A <-merge(A1,A2,by="V2")[,c(1:5,8,9)]
conds_A <- c("NC_A1","A1","NC_A2","A2")
names(A) <- c("sgRNA", "gene_symbol","seq",conds_A)
B <-merge(B1,B2,by="V2")[,c(1:5,8,9)]
conds_B <- c("NC_B1","B1","NC_B2","B2")
names(B) <- c("sgRNA", "gene_symbol","seq",conds_B)
...
###########  A  ######################
dcpm <- cpm(as.matrix(A[, conds_A]))
dfcpm <- as.data.frame(dcpm+0.1)
rownames(dfcpm) <- NULL
head(dfcpm)

dfm <- melt(dfcpm, measure.vars=1:4, variable.name="conds", value.name="count")
...
q
ggsave(paste(dir,"A_histCpm_1.pdf"), q, width=4, height=3)
################  B #############################
...
q <- ggplot(data=dfm, aes(x=count, ..density.., group=conds, colour=conds)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size=12))
q
ggsave(paste(dir,"B_histCpm_1.pdf"), q, width=4, height=3)
